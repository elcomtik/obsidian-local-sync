...
| BUG-22 | `sqliteDriver.ts`, `main.ts` | Plugin failed to load on Android: `sqliteDriver.ts` imported `node:fs` and `node:path` directly; `main.ts` used `FileSystemAdapter.getBasePath()` + `path.join()`. Android's WebView has no Node.js runtime, so these imports threw on module evaluation. Fixed by replacing all `fs`/`path` usage with a `PlatformIO` interface (`readFile`/`writeFile`) backed by `app.vault.adapter.readBinary/writeBinary` — Obsidian's cross-platform DataAdapter API, available on both desktop and mobile. | ✅ 0.1.1 |
| BUG-23 | `esbuild.config.mjs` | `ReferenceError: process is not defined` on Android: esbuild `platform:"node"` resolved `msgpackr` to its Node.js entry point (`node-index.js`), which evaluates `process.env.MSGPACKR_NATIVE_ACCELERATION_DISABLED` at module load time. On Android `process` is not a global, so the bundle crashed before any plugin code ran. A `globalThis.process` polyfill in the banner was insufficient because the reference was emitted directly as an identifier in the module scope. Fixed by switching esbuild to `platform:"browser"`, causing it to use msgpackr's `browser` export condition (`./index.js`) which contains no Node globals. All remaining node built-ins (`node:fs`, `node:crypto`, etc.) are explicitly marked as external and are only reached inside sql.js's `if(ca)` guard (`ca = globalThis.process?.versions?.node`) which is always falsy on Android. | ✅ 0.1.1 |
| BUG-24 | `engine.ts` | Delete echo loop on seeding: when `applyFileUpdateRowById` processed a remote delete and called `vault.trash()`, the vault `"delete"` event fired synchronously, triggering `onVaultFileDeleted`, which emitted a new `fileUpdate { type: "delete" }` row. The new row was picked up by other devices on their next poll — a file deleted-then-recreated on another device would be deleted again by the echoed row from the seeding client. Same class of bug as BUG-17 (content echo loop). Fixed by adding `pendingRemoteDeletes: Set<string>` to `YjsEvoluHistoryEngine`: the path is added before `vault.trash()` and removed in a `finally` block; `onVaultFileDeleted` returns early if the path is in this set. | ✅ 0.1.2 |
| BUG-25 | `main.ts` | Destructive local reset/restore preserved `deviceId`. The fresh peer then filtered all relay rows originally authored by that same device as live self-echoes, rebuilding only paths authored by other peers. Fixed by rotating and persisting the peer ID after the local wipe and before reconnecting. | ✅ 0.8.2 |
...
| ARCH-2 | `engine.ts` | N+1 query: each history row triggered a separate `applyFileUpdateRowById` DB query. Up to `historyBatchSize` (default 500) individual DB queries per poll. Fixed: `pollHistoryOnce` now uses 2 queries — `evolu_history` for IDs, then `fileUpdate WHERE id IN (...)` for all row fields. A JOIN was not viable: `evolu_history.id` is BLOB while `fileUpdate.id` is TEXT, so the join predicate never matches in SQLite. `applyFileUpdateRowById` removed; logic inlined. Look-ahead pass records the index of the last delete per path; content writes are skipped only when a delete appears at a LATER index (not when the delete is earlier, which is a delete-then-recreate sequence). Eliminates the Obsidian async metadata race (`computeFileMetadataAsync` after `vault.modify()` racing a subsequent `vault.trash()`). | ✅ 0.1.3 |
