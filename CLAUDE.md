...
| ARCH-2 | `engine.ts:260` | N+1 query: each history row triggers a separate `applyFileUpdateRowById` query. Up to `historyBatchSize` (default 500) individual DB queries per poll. A join query would collapse this to 1. Side-effect: when a content update and a delete for the same path appear in the same batch, `vault.modify()` and `vault.trash()` execute back-to-back. `vault.modify()` returns once the fs write is done, but Obsidian fires its internal metadata pipeline (`computeFileMetadataAsync`) asynchronously — not as part of the awaited Promise. The subsequent `vault.trash()` removes the file while that metadata job is still running, causing Obsidian to log `"File does not exist"` from its own internals. Cosmetic only (end state is correct). A join query that returns `type` + `path` for all rows upfront would allow skipping `vault.modify()` for files whose batch also contains a delete, eliminating the race entirely. Duplicate delete rows for the same path are already handled safely: the second delete finds `getAbstractFileByPath → null` and skips `vault.trash()`. | ⚠ open |
