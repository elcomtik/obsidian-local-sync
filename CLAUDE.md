...
| ARCH-4 | `engine.ts` | No `rename` or `delete` vault event handlers. Renaming a file leaves the old path's Yjs doc alive until LRU evicts it; the new path starts with a fresh doc. File deletion on one device is not propagated to other devices. Acknowledged in roadmap. | ⚠ open (Test I) |
